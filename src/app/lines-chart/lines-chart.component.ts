...
import { Axis, AxisDomain, gray } from 'd3';
...
export class LinesChartComponent implements OnInit {
  ...
  get width(): number {
    return parseInt(d3.select('.container').style('width'), 10) - 100;
  }
  ...
  ngOnInit() {
    this.dataSource = <Country[]>this.service.getData();
    this.total = this.dataSource.reduce((sum, it) => sum += it.value, 0);
    this.svg = d3.select('#lines').select('svg');
    this.xScale = d3.scaleBand();
    this.yScale = d3.scaleLinear();
    this.initSvg();

    this.tooltip = d3.select('#lines')
      .append('div')
      .attr('class', 'tooltip')
      .style('display', 'none')
      .style('opacity', 0);
  ...
  private drawAxis() {
    this.gy.attr('transform', `translate(5, 0)`).call(this.yAxis);
